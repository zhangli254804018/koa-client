...
class UploadController {
    ...
    static async post(ctx) {
        ...
        if (query.uid && !errmsg && !posted.result) {
            await uploadModel.create(query, function(err) {
                if (err) return Util.reponse(ctx, -1, '', err)
            }).then(function(doc) {
                query.aid = doc.id
                query.token = md5(doc.id)
            })
            return await uploadModel.findByIdAndUpdate(query.aid, { $set: query }, { new: true }).exec().then(function(doc) {
    ...
    static async posted(ctx) {
        ...
        if (!_.isEmpty(obj)) {
            return await uploadModel.findOne(obj, function(err) {
                if (err) return Util.reponse(ctx, -1)
    ...
    static async update(ctx) {
        ...
        if (posted.result && !errmsg && obj.aid) {
            return await uploadModel.findByIdAndUpdate(obj.aid, { $set: query }, { new: true }).exec().then(function(doc) {
                return doc ? Util.reponse(ctx, 0, vm.init(doc)) : Util.reponse(ctx, -1)
            }).catch(function(err) {
                if (err) return Util.reponse(ctx, -1)
            })
    ...
    static async remove(ctx) {
        ...
        obj.aid = query.aid
        if (obj.aid) {
            return await uploadModel.findOneAndRemove(obj, function(err, article) {
